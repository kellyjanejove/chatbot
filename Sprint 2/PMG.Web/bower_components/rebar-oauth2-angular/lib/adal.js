    ...
    */
    this.CONSTANTS = {
        ...
        STORAGE: {
			...
			OAUTH1_ACCESS_TOKEN_KEY: 'adal.oatuh1.token.key',
            EXPIRATION_KEY: 'adal.expiration.key',
            START_PAGE: 'adal.start.page',
			OAUTH_REDIRECT_URL: 'adal.oauth.redirect.url',
...
AuthenticationContext.prototype.clearCache = function () {
	...
	this._saveItem(this.CONSTANTS.STORAGE.OAUTH_REDIRECT_URL, '');
...
AuthenticationContext.prototype.isCallback = function (hash) {
    ...
    return (
			...
			parameters.hasOwnProperty(this.CONSTANTS.STATE) ||
...
AuthenticationContext.prototype.saveTokenFromHash = function (requestInfo) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION) ||
			requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR)) {
        ...
        this._saveItem(this.CONSTANTS.STORAGE.FAILED_RENEW, requestInfo.parameters.error+" "+requestInfo.parameters[this.CONSTANTS.ERROR_DESCRIPTION]);
        ...
        if (requestInfo.stateMatch) {
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
				...
				//Clear oauth1 token cache so that it refreshes oauth1 token on the next request because the bootstrap oauth2 token is renewed
				this._clearOAuth1TokenCache();
            }
...
/* //This logic is meant only for AD...not required for OIDC implicit flow with IdenityServer
...
AuthenticationContext.prototype._clearOAuth1TokenCache = function () {
    var context= this;
	angular.forEach(context.config.oauth1ServicesAndIdentifierMap, function (value, key) {
		var identifier = key;
		var serviceBaseUrl = value;		
		context._saveItem(context.CONSTANTS.STORAGE.OAUTH1_ACCESS_TOKEN_KEY+identifier,'');
	});
};

AuthenticationContext.prototype._cacheOAuth1Token = function (identifier,token) {	

		this._saveItem(this.CONSTANTS.STORAGE.OAUTH1_ACCESS_TOKEN_KEY+identifier,token);

		};
		
AuthenticationContext.prototype._getOAuth1Token = function (identifier) {	

		return this._getItem(this.CONSTANTS.STORAGE.OAUTH1_ACCESS_TOKEN_KEY+identifier);

		};		

// AuthenticationContext.prototype._fetchOAuth1Token = function (identifier, ouath2Token) {
...
AuthenticationContext.prototype._IsOAuth1Request = function(requestUrl) {
						var found=null;
					     angular.forEach(this.config.oauth1ServicesAndIdentifierMap, function (value, key) {
							var identifier = key;
							var serviceBaseUrl = value;						 
						    if (requestUrl.toLowerCase().match(serviceBaseUrl.toLowerCase())) {
								found=identifier;
